...
Release Notes: xref::en/mule-teradata-connector/release-notes.adoc[Teradata Connector Release Notes]
...
* <<config_teradata, Teradata Connection>>
...
NOTE: To specify an SQL function in an SQL query in an operation, specify the SQL function in the `{fn function()}` format. For example, the SQL function `CURRENT_TIMESTAMP` is specified as `{fn CURRENT_TIMESTAMP()}`.
