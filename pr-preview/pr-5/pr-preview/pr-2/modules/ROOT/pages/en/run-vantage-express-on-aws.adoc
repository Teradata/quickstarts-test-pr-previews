...
If you want a cheaper option, try xref::en/vantage.express.gcp.adoc[Google Cloud] and xref::en/run-vantage-express-on-microsoft-azure.adoc[Azure] which support nested virtualization and can run Vantage Express on cheap VM's.

If you do not wish to pay for cloud usage at all, install Vantage Express locally using xref::en/getting.started.vmware.adoc[VMware], xref::en/getting.started.vbox.adoc[VirtualBox], or xref::en/getting.started.utm.adoc[UTM].
====
