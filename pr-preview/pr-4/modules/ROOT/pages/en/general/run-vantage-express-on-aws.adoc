...
If you want a cheaper option, try xref::en/general/vantage.express.gcp.adoc[Google Cloud] and xref::en/general/run-vantage-express-on-microsoft-azure.adoc[Azure] which support nested virtualization and can run Vantage Express on cheap VM's.

If you do not wish to pay for cloud usage at all, install Vantage Express locally using xref::en/general/getting.started.vmware.adoc[VMware], xref::en/general/getting.started.vbox.adoc[VirtualBox], or xref::en/general/getting.started.utm.adoc[UTM].
====
