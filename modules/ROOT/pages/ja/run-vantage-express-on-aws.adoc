...
より安価なオプションが必要な場合は、ネストされた仮想化をサポートし、安価な VM 上で Vantage Express を実行できる xref::ja/vantage.express.gcp.adoc[Google Cloud] と xref:ja/run-vantage-express-on-microsoft-azure.adoc[Azure] をお試しください。
