...
より安価なオプションが必要な場合は、ネストされた仮想化をサポートし、安価な VM 上で Vantage Express を実行できる xref::ja/general/vantage.express.gcp.adoc[Google Cloud] と xref::ja/general/run-vantage-express-on-microsoft-azure.adoc[Azure] をお試しください。
